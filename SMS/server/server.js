...
app.get("/students", async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM public."StudentsData" ORDER BY "RegisterationNo" ASC');
...
app.post("/students", async (req, res) => {
    try {
        ...
        const newStudent = await pool.query(
            'INSERT INTO public."StudentsData"("Name", "RegisterationNo", "Department", "Course", "picUrl", "CGPA") VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
...
app.delete("/students/:registerationNo", async (req, res) => {
    ...
    try {
        const result = await pool.query(
            'DELETE FROM public."StudentsData" WHERE "RegisterationNo" = $1 RETURNING *',
            [registerationNo]
        );
